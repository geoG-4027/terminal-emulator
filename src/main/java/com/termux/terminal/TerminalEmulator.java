...
/**
 * An abstract terminal emulator which aims to be xterm compatible where it makes sense (for features used by modern
 * command line apps).
 * <p>
 * References:
 * <ul>
 * <li>http://invisible-island.net/xterm/ctlseqs/ctlseqs.html - XTerm Control Sequences</li>
 ...
 * <li>http://www.madonnadeipoveri.org/opt/kde/share/doc/HTML/en/konsole/techref.html - for konsole</li>
 * </ul>
 */
public final class TerminalEmulator {
    ...
    /** Parameter to {@link #sendMouseEvent(int, int, int, boolean)} for when the left mouse button has been pressed. */
    public static final int MOUSE_LEFT_BUTTON_PRESSED = 0;

    /** Parameter to {@link #sendMouseEvent(int, int, int, boolean)} for when the mouse has been dragged while the left mouse button has been pressed. */
    ...
    /** The terminal title stack, which allows programs to restore the title on exit. */
    private final Stack<String> titleStack = new Stack<>();

    /** The cursor position. Between (0,0) and (rows-1, columns-1). */
    private int cursorRow, cursorCol;

    public enum CursorStyle { CURSOR_STYLE_BLOCK, CURSOR_STYLE_UNDERLINE, CURSOR_STYLE_BAR }

    private CursorStyle cursorStyle = CursorStyle.CURSOR_STYLE_BLOCK;

    /** The number of character rows and columns in the terminal screen. */
    int rows, columns;

    /** The normal screen buffer. Stores the characters that appear on the screen of the emulated terminal. */
    private final TerminalBuffer mainBuffer;
    /**
     * The alternate screen buffer, exactly as large as the display and contains no additional saved rows (so that when
     * the alternate screen buffer is active, you cannot scroll back to view saved rows).
     * <p>
     * See http://www.xfree86.org/current/ctlseqs.html#The%20Alternate%20Screen%20Buffer
     */
    final TerminalBuffer altBuffer;

    /** The current screen buffer, pointing at either {@link #mainBuffer} or {@link #altBuffer}. */
    private TerminalBuffer screen;

    /** The terminal session this emulator is bound to. */
    private final TerminalClient session;
    ...
    private final int[] currentArgs = new int[MAX_ESCAPE_PARAMETERS];

    /** Holds OSC and device control arguments, which can be strings. */
    private final StringBuilder oscOrDeviceControlArgs = new StringBuilder();
    ...
    private final SavedScreenState savedStateMain = new SavedScreenState();
    private final SavedScreenState savedStateAlt = new SavedScreenState();

    /** http://www.vt100.net/docs/vt102-ug/table5-15.html */
    private boolean useLineDrawingG0, useLineDrawingG1, useLineDrawingUsesG0 = true;

    /**
     * @see TerminalEmulator#mapDecSetBitToInternalBit(int)
     */
    private int currentDecSetFlags, savedDecSetFlags;
    ...
    /**
     * The number of scrolled rows since last calling {@link #clearScrollCounter()}. Used for moving selection up along
     * with the scrolling text.
     */
    private int scrollCounter = 0;

    private byte utf8ToFollow, utf8Index;
    private final byte[] utf8InputBuffer = new byte[4];
    private int lastEmittedCodePoint = -1;

    public final TerminalColors colors = new TerminalColors();

    private final PrintStream logger;

    private boolean isDecsetInternalBitSet(int bit) {
        return (currentDecSetFlags & bit) != 0;
    }
    ...
    private void setDecsetinternalBit(int internalBit, boolean set) {
        ...
        if (set) {
            currentDecSetFlags |= internalBit;
        } else {
            currentDecSetFlags &= ~internalBit;
        }
    }
    ...
    public TerminalEmulator(TerminalClient session, int columns, int rows, int transcriptRows, PrintStream logger) {
        this.logger = logger;
        this.session = session;
        screen = mainBuffer = new TerminalBuffer(columns, transcriptRows, rows);
        altBuffer = new TerminalBuffer(columns, rows, rows);
        this.rows = rows;
    ...
    public void sendMouseEvent(int mouseButton, int column, int row, boolean pressed) {
        ...
        if (mouseButton == MOUSE_LEFT_BUTTON_MOVED && !isDecsetInternalBitSet(DECSET_BIT_MOUSE_TRACKING_BUTTON_EVENT)) {
            ...
            session.write(String.format("\033[<%d;%d;%d" + (pressed ? 'M' : 'm'), mouseButton, column, row));
        } else {
    ...
    private void resizeScreen() {
        final int[] cursor = {cursorCol, cursorRow};
        int newTotalRows = (screen == altBuffer) ? rows : mainBuffer.totalRows;
    ...
    private void processByte(byte byteToProcess) {
        if (utf8ToFollow > 0) {
            if ((byteToProcess & 0b11000000) == 0b10000000) {
                ...
                utf8InputBuffer[utf8Index++] = byteToProcess;
                if (--utf8ToFollow == 0) {
                    byte firstByteMask = (byte) (utf8Index == 2 ? 0b00011111 : (utf8Index == 3 ? 0b00001111 : 0b00000111));
                    int codePoint = (utf8InputBuffer[0] & firstByteMask);
                    for (int i = 1; i < utf8Index; i++)
                        codePoint = ((codePoint << 6) | (utf8InputBuffer[i] & 0b00111111));
            ...
            default:
                ...
                switch (escapeState) {
                    ...
                    case ESC_CSI_DOLLAR:
                        ...
                        int effectiveTopMargin = originMode ? topMargin : 0;
                        int effectiveBottomMargin = originMode ? bottomMargin : rows;
                        int effectiveLeftMargin = originMode ? leftMargin : 0;
                        int effectiveRightMargin = originMode ? rightMargin : columns;
                        switch (b) {
                            case 'v': // ${CSI}${SRC_TOP}${SRC_LEFT}${SRC_BOTTOM}${SRC_RIGHT}${SRC_PAGE}${DST_TOP}${DST_LEFT}${DST_PAGE}$v"
                                ...
                                int topSource = Math.min(getArg(0, 1, true) - 1 + effectiveTopMargin, rows);
                                int leftSource = Math.min(getArg(1, 1, true) - 1 + effectiveLeftMargin, columns);
                                // Inclusive, so do not subtract one:
                                int bottomSource = Math.min(Math.max(getArg(2, rows, true) + effectiveTopMargin, topSource), rows);
                                int rightSource = Math.min(Math.max(getArg(3, columns, true) + effectiveLeftMargin, leftSource), columns);
                                // int sourcePage = getArg(4, 1, true);
                                int destionationTop = Math.min(getArg(5, 1, true) - 1 + effectiveTopMargin, rows);
                                int destinationLeft = Math.min(getArg(6, 1, true) - 1 + effectiveLeftMargin, columns);
                                // int destinationPage = getArg(7, 1, true);
                                int heightToCopy = Math.min(rows - destionationTop, bottomSource - topSource);
                                int widthToCopy = Math.min(columns - destinationLeft, rightSource - leftSource);
                            ...
                            case 'z': // ${CSI}$${TOP}${LEFT}${BOTTOM}${RIGHT}$z"
                                ...
                                if ((fillChar >= 32 && fillChar <= 126) || (fillChar >= 160 && fillChar <= 255)) {
                                    ...
                                    int bottom = Math.min(getArg(argIndex++, rows, true) + effectiveTopMargin, effectiveBottomMargin);
                                    int right = Math.min(getArg(argIndex, columns, true) + effectiveLeftMargin, effectiveRightMargin);
                                    long style = getStyle();
                                    for (int row = top - 1; row < bottom; row++)
                                        for (int col = left - 1; col < right; col++)
                                            if (!selective || (TextStyle.decodeEffect(screen.getStyleAt(row, col)) & TextStyle.CHARACTER_ATTRIBUTE_PROTECTED) == 0)
                            ...
                            case 't': // "${CSI}${TOP}${LEFT}${BOTTOM}${RIGHT}${ATTRIBUTES}$t"
                                ...
                                int bottom = Math.min(getArg(2, rows, true) + 1, effectiveBottomMargin - 1) + effectiveTopMargin;
                                int right = Math.min(getArg(3, columns, true) + 1, effectiveRightMargin - 1) + effectiveLeftMargin;
                                if (this.argIndex >= 4) {
                                    if (this.argIndex >= currentArgs.length) this.argIndex = currentArgs.length - 1;
                                    for (int i = 4; i <= this.argIndex; i++) {
                                        int bits = 0;
                        ...
                        if (b == 'q') {
                            ...
                            switch (arg) {
                                ...
                                case 2:
                                    ...
                                    effect &= ~TextStyle.CHARACTER_ATTRIBUTE_PROTECTED;
                                    break;
                                case 1:
                    ...
                    case ESC_CSI_SINGLE_QUOTE:
                        switch (b) {
                            case '}': { // Insert Ps Column(s) (default = 1) (DECIC), VT420 and up.
                                int columnsAfterCursor = rightMargin - cursorCol;
                            ...
                            case '~': { // Delete Ps Column(s) (default = 1) (DECDC), VT420 and up.
                                ...
                                blockClear(cursorRow + columnsToMove, 0, columnsToDelete, rows);
                                break;
                            }
                            default:
                                unknownSequence(b);
                        ...
                        switch (b) {
                            case 'q': // "${CSI}${STYLE} q" - set cursor style (http://www.vt100.net/docs/vt510-rm/DECSCUSR).
                                switch (arg) {
                                    ...
                                    case 2: // Steady block.
                                        cursorStyle = CursorStyle.CURSOR_STYLE_BLOCK;
                                        break;
                                    case 3: // Blinking underline.
                                    case 4: // Steady underline.
                                        cursorStyle = CursorStyle.CURSOR_STYLE_UNDERLINE;
                                        break;
                                    case 5: // Blinking bar (xterm addition).
                                    case 6: // Steady bar (xterm addition).
                                        cursorStyle = CursorStyle.CURSOR_STYLE_BAR;
                                        break;
                                }
                                break;
                            case 't':
    ...
    private void doDeviceControl(int b) {
        switch (b) {
            ...
            {
                String dcs = oscOrDeviceControlArgs.toString();
                ...
                if (dcs.startsWith("$q")) {
                    if (dcs.equals("$q\"p")) {
                        ...
                        session.write("\033P1$r" + csiString + "\033\\");
                    } else {
                            ...
                            if (responseValue == null) {
                                switch (trans) {
                                    ...
                                    case "&8": // Undo key - ignore.
                                        break;
                                    default:
                                        logger.println("Unhandled termcap/terminfo name: '" + trans + "'");
                                }
                                // Respond with invalid request:
                                session.write("\033P0+r" + part + "\033\\");
                            } else {
                                ...
                                session.write("\033P1+r" + part + "=" + hexEncoded + "\033\\");
                            }
                        } else {
                            logger.println("Invalid device termcap/terminfo name of odd length: " + part);
                        }
                    }
                } else {
                    if (LOG_ESCAPE_SEQUENCES)
                        logger.println("Unrecognized device control string: " + dcs);
                }
                finishSequence();
            }
            break;
    ...
    private int nextTabStop(int numTabs) {
        for (int i = cursorCol + 1; i < columns; i++)
            if (tabStop[i] && --numTabs == 0) return Math.min(i, rightMargin);
    ...
    private void doCsiQuestionMark(int b) {
        switch (b) {
            ...
            case 'K': // Selective erase in line (DECSEL) - http://vt100.net/docs/vt510-rm/DECSEL.
                ...
                switch (getArg0(0)) {
                    case 0: // Erase from the active position to the end, inclusive (default).
                        ...
                        startRow = cursorRow;
                        endCol = columns;
                        endRow = justRow ? (cursorRow + 1) : rows;
                        break;
                    case 1: // Erase from start to the active position, inclusive.
                        startCol = 0;
                        startRow = justRow ? cursorRow : 0;
                        endCol = cursorCol + 1;
                        endRow = cursorRow + 1;
                        break;
                ...
                for (int row = startRow; row < endRow; row++) {
                    for (int col = startCol; col < endCol; col++) {
                        if ((TextStyle.decodeEffect(screen.getStyleAt(row, col)) & TextStyle.CHARACTER_ATTRIBUTE_PROTECTED) == 0)
            ...
            case 'h':
            case 'l':
                if (argIndex >= currentArgs.length) argIndex = currentArgs.length - 1;
                for (int i = 0; i <= argIndex; i++)
                    doDecSetOrReset(b == 'h', currentArgs[i]);
                break;
            case 'n': // Device Status Report (DSR, DEC-specific).
                switch (getArg0(-1)) {
                    case 6:
                        ...
                        session.write(String.format(Locale.US, "\033[?%d;%d;1R", cursorRow + 1, cursorCol + 1));
                        break;
                    default:
                        finishSequence();
                        return;
            ...
            case 's':
                if (argIndex >= currentArgs.length) argIndex = currentArgs.length - 1;
                for (int i = 0; i <= argIndex; i++) {
                    int externalBit = currentArgs[i];
                    int internalBit = mapDecSetBitToInternalBit(externalBit);
                    if (internalBit == -1) {
                        logger.println("Ignoring request to save/recall decset bit=" + externalBit);
                    } else {
                        if (b == 's') {
                            savedDecSetFlags |= internalBit;
                        } else {
                            doDecSetOrReset((savedDecSetFlags & internalBit) != 0, externalBit);
                        }
                    }
                }
                break;
    ...
    public void doDecSetOrReset(boolean setting, int externalBit) {
        ...
        switch (externalBit) {
            ...
            case 1049: {
                ...
                TerminalBuffer newScreen = setting ? altBuffer : mainBuffer;
                if (newScreen != screen) {
                    boolean resized = !(newScreen.columns == columns && newScreen.screenRows == rows);
                    if (setting) saveCursor();
                    screen = newScreen;
                    if (!setting) {
                        int col = savedStateMain.savedCursorCol;
                        int row = savedStateMain.savedCursorRow;
                        restoreCursor();
                        if (resized) {
                    ...
                    if (newScreen == altBuffer)
                        newScreen.blockSet(0, 0, columns, rows, ' ', getStyle());
                }
                break;
            }
            case 2004:
    ...
    private void doCsiBiggerThan(int b) {
        switch (b) {
            ...
            case 'm':
                ...
                logger.println("(ignored) CSI > MODIFY RESOURCE: " + getArg0(-1) + " to " + getArg1(-1));
                break;
            default:
                parseArg(b);
                break;
    ...
    private void startEscapeSequence() {
        escapeState = ESC;
        argIndex = 0;
        Arrays.fill(currentArgs, -1);
    }

    private void doLinefeed() {
        boolean belowScrollingRegion = cursorRow >= bottomMargin;
    ...
    private void doEscPound(int b) {
        switch (b) {
            case '8': // Esc # 8 - DEC screen alignment test - fill screen with E's.
                screen.blockSet(0, 0, columns, rows, 'E', getStyle());
                break;
            default:
                unknownSequence(b);
                break;
        ...
        switch (b) {
            ...
            case '6': // Back index (http://www.vt100.net/docs/vt510-rm/DECBI). Move left, insert blank column if start.
                if (cursorCol > leftMargin) {
                    ...
                    int rows = bottomMargin - topMargin;
                    screen.blockCopy(leftMargin, topMargin, rightMargin - leftMargin - 1, rows, leftMargin + 1, topMargin);
                    screen.blockSet(leftMargin, topMargin, 1, rows, ' ', TextStyle.encode(foreColor, backColor, 0));
                }
                break;
            ...
            case '9': // Forward Index (http://www.vt100.net/docs/vt510-rm/DECFI). Move right, insert blank column if end.
                if (cursorCol < rightMargin - 1) {
                    ...
                    screen.blockSet(rightMargin - 1, topMargin, 1, rows, ' ', TextStyle.encode(foreColor, backColor, 0));
                }
                break;
            ...
            case 'E': // Next line (http://www.vt100.net/docs/vt510-rm/NEL).
                setCursorCol(isDecsetInternalBitSet(DECSET_BIT_ORIGIN_MODE) ? leftMargin : 0);
                doLinefeed();
                break;
            ...
            case 'M': // "${ESC}M" - reverse index (RI).
                ...
                if (cursorRow <= topMargin) {
                    screen.blockCopy(0, topMargin, columns, bottomMargin - (topMargin + 1), 0, topMargin + 1);
    ...
    private void saveCursor() {
        SavedScreenState state = (screen == mainBuffer) ? savedStateMain : savedStateAlt;
    ...
    private void restoreCursor() {
        ...
        currentDecSetFlags = (currentDecSetFlags & ~mask) | (state.savedDecFlags & mask);
    ...
    private void doCsi(int b) {
        switch (b) {
            ...
            case '@': {
                ...
                aboutToAutoWrap = false;
                int columnsAfterCursor = columns - cursorCol;
            ...
            case 'a': // "CSI${n}a" - Horizontal position relative (HPR). From ISO-6428/ECMA-48.
                setCursorCol(Math.min(rightMargin - 1, cursorCol + getArg0(1)));
                break;
            case 'D': // "CSI${n}D" - Cursor backward (CUB) ${n} columns.
                setCursorCol(Math.max(leftMargin, cursorCol - getArg0(1)));
                break;
            ...
            case 'J': // "${CSI}${0,1,2}J" - Erase in Display (ED)
                ...
                switch (getArg0(0)) {
                    ...
                    case 1: // Erase from start of the screen to the active position, inclusive.
                        ...
                        break;
                    case 2: // Erase all of the display - all rows are erased, changed to single-width, and the cursor does not
                        // move..
                        blockClear(0, 0, columns, rows);
                        break;
                    default:
            ...
            case 'K': // "CSI{n}K" - Erase in line (EL).
                ...
                aboutToAutoWrap = false;
                break;
            case 'L': // "${CSI}{N}L" - insert ${N} rows (IL).
            {
            ...
            break;
            case 'M': // "${CSI}${N}M" - delete N rows (DL).
            {
                aboutToAutoWrap = false;
                int linesAfterCursor = bottomMargin - cursorRow;
            ...
            case 'P': // "${CSI}{N}P" - delete ${N} characters (DCH).
            {
                ...
                aboutToAutoWrap = false;
                int cellsAfterCursor = columns - cursorCol;
            ...
            break;
            case 'S': { // "${CSI}${N}S" - scroll up ${N} rows (default = 1) (SU).
            ...
            case 'T':
                if (argIndex == 0) {
                    // "${CSI}${N}T" - Scroll down N rows (default = 1) (SD).
                    // http://vt100.net/docs/vt510-rm/SD: "N is the number of rows to move the user window up in page
                    // memory. N new rows appear at the top of the display. N old rows disappear at the bottom of the
                    // display. You cannot pan past the top margin of the current page".
                    final int linesToScrollArg = getArg0(1);
                    final int linesBetweenTopAndBottomMargins = bottomMargin - topMargin;
                    final int linesToScroll = Math.min(linesBetweenTopAndBottomMargins, linesToScrollArg);
                    screen.blockCopy(0, topMargin, columns, linesBetweenTopAndBottomMargins - linesToScroll, 0, topMargin + linesToScroll);
                    blockClear(0, topMargin, columns, linesToScroll);
                } else {
            ...
            case 'X': // "${CSI}${N}X" - Erase ${N:=1} character(s) (ECH).
                aboutToAutoWrap = false;
                screen.blockSet(cursorCol, cursorRow, Math.min(getArg0(1), columns - cursorCol), 1, ' ', getStyle());
                break;
            case 'Z': // Cursor Backward Tabulation (CBT). Move the active position n tabs backward.
                int numberOfTabs = getArg0(1);
                int newCol = leftMargin;
                for (int i = cursorCol - 1; i >= 0; i--)
                    if (tabStop[i]) {
                        if (--numberOfTabs == 0) {
                            newCol = Math.max(i, leftMargin);
                            break;
                        }
                    }
                cursorCol = newCol;
            ...
            case 'b': // Repeat the preceding graphic character Ps times (REP).
                if (lastEmittedCodePoint == -1) break;
                final int numRepeat = getArg0(1);
                for (int i = 0; i < numRepeat; i++) emitCodePoint(lastEmittedCodePoint);
                break;
            case 'c': // Primary Device Attributes (http://www.vt100.net/docs/vt510-rm/DA1) if argument is missing or zero.
                ...
                if (getArg0(0) == 0) session.write("\033[?64;1;2;6;9;15;18;21;22c");
                break;
            ...
            case 'n': // Esc [ Pn n - ECMA-48 Status Report Commands
                ...
                switch (getArg0(0)) {
                    ...
                    case 6: // Cursor position report (CPR):
                        ...
                        // the cursor location.
                        session.write(String.format(Locale.US, "\033[%d;%dR", cursorRow + 1, cursorCol + 1));
                        break;
                    default:
                        break;
                }
            ...
            case 'r': // "CSI${top};${bottom}r" - set top and bottom Margins (DECSTBM).
            {
                ...
                topMargin = Math.max(0, Math.min(getArg0(1) - 1, rows - 2));
                bottomMargin = Math.max(topMargin + 2, Math.min(getArg1(rows), rows));
            ...
            case 's':
                if (isDecsetInternalBitSet(DECSET_BIT_LEFTRIGHT_MARGIN_MODE)) {
                    ...
                    leftMargin = Math.min(getArg0(1) - 1, columns - 2);
                    rightMargin = Math.max(leftMargin + 1, Math.min(getArg1(columns), columns));
            ...
            case 't': // Window manipulation (from dtterm, as well as extensions)
                switch (getArg0(0)) {
                    ...
                    case 23: // Like 22 above but restore from stack.
                        if (!titleStack.isEmpty()) setTitle(titleStack.pop());
                        break;
                    default:
    ...
    private void selectGraphicRendition() {
        ...
        for (int i = 0; i <= argIndex; i++) {
            ...
            if (code == 0) { // reset
                ...
                effect |= TextStyle.CHARACTER_ATTRIBUTE_BOLD;
            } else if (code == 2) {
                effect |= TextStyle.CHARACTER_ATTRIBUTE_DIM;
            } else if (code == 3) {
                effect |= TextStyle.CHARACTER_ATTRIBUTE_ITALIC;
            } else if (code == 4) {
                effect |= TextStyle.CHARACTER_ATTRIBUTE_UNDERLINE;
            } else if (code == 5) {
                effect |= TextStyle.CHARACTER_ATTRIBUTE_BLINK;
            } else if (code == 7) {
                effect |= TextStyle.CHARACTER_ATTRIBUTE_INVERSE;
            } else if (code == 8) {
                effect |= TextStyle.CHARACTER_ATTRIBUTE_INVISIBLE;
            } else if (code == 9) {
                effect |= TextStyle.CHARACTER_ATTRIBUTE_STRIKETHROUGH;
            } else if (code == 10) {
                ...
                effect &= ~(TextStyle.CHARACTER_ATTRIBUTE_BOLD | TextStyle.CHARACTER_ATTRIBUTE_DIM);
                ...
                if (i + 2 > argIndex) continue;
                int firstArg = currentArgs[i + 1];
                switch (firstArg) {
                    case 2:
                        if (i + 4 > argIndex) {
                            logger.println("Too few CSI" + code + ";2 RGB arguments");
                        } else {
                            int red = currentArgs[i + 2], green = currentArgs[i + 3], blue = currentArgs[i + 4];
                    ...
                    case 5:
                        ...
                        if (color >= 0 && color < TextStyle.NUM_INDEXED_COLORS) {
                            ...
                            if (LOG_ESCAPE_SEQUENCES) logger.println("Invalid color index: " + color);
                        }
                        break;
                    default:
                ...
                if (LOG_ESCAPE_SEQUENCES)
                    logger.println(String.format("SGR unknown code %d", code));
            }
        }
    }
    ...
    private void doOscSetTextParameters(String bellOrStringTerminator) {
        ...
        for (int mOSCArgTokenizerIndex = 0; mOSCArgTokenizerIndex < oscOrDeviceControlArgs.length(); mOSCArgTokenizerIndex++) {
            char b = oscOrDeviceControlArgs.charAt(mOSCArgTokenizerIndex);
            if (b == ';') {
                textParameter = oscOrDeviceControlArgs.substring(mOSCArgTokenizerIndex + 1);
                break;
            } else if (b >= '0' && b <= '9') {
        ...
        switch (value) {
                    ...
                    if (endOfInput || textParameter.charAt(charIndex) == ';') {
                        try {
                            ...
                            if ("?".equals(colorSpec)) {
                                ...
                                int rgb = colors.currentColors[specialIndex];
                                int r = (65535 * ((rgb & 0x00FF0000) >> 16)) / 255;
                                int g = (65535 * ((rgb & 0x0000FF00) >> 8)) / 255;
                                int b = (65535 * ((rgb & 0x000000FF))) / 255;
                                session.write("\033]" + value + ";rgb:" + String.format(Locale.US, "%04x", r) + "/" + String.format(Locale.US, "%04x", g) + "/"
            ...
            case 52: // Manipulate Selection Data. Skip the optional first selection parameter(s).
                ...
                try {
                    String clipboardText = new String(Base64.getDecoder().decode(textParameter.substring(startIndex)), StandardCharsets.UTF_8);
                    session.clipboardText(clipboardText);
                } catch (Exception e) {
                    logger.println("OSC Manipulate selection, invalid string '" + textParameter + "");
                }
                break;
            case 104:
    ...
    private void scrollDownOneLine() {
        ...
        if (leftMargin != 0 || rightMargin != columns) {
            ...
            screen.blockSet(leftMargin, bottomMargin - 1, rightMargin - leftMargin, 1, ' ', effect);
        } else {
            screen.scrollDownOneLine(topMargin, bottomMargin, getStyle());
        }
    }
    ...
    private void parseArg(int b) {
        if (b >= '0' && b <= '9') {
            if (argIndex < currentArgs.length) {
                int oldValue = currentArgs[argIndex];
                int thisDigit = b - '0';
                int value;
                if (oldValue >= 0) {
    ...
    private void finishSequenceAndLogError(String error) {
        if (LOG_ESCAPE_SEQUENCES) logger.println(error);
        finishSequence();
    }
    ...
    private void emitCodePoint(int codePoint) {
        ...
        if (useLineDrawingUsesG0 ? useLineDrawingG0 : useLineDrawingG1) {
            // http://www.vt100.net/docs/vt102-ug/table5-15.html.
            switch (codePoint) {
                case '_':
                    codePoint = ' '; // Blank.
        ...
        final boolean cursorInLastColumn = cursorCol == rightMargin - 1;
        ...
        if (insertMode && displayWidth > 0) {
            ...
            int destCol = cursorCol + displayWidth;
            if (destCol < rightMargin)
                screen.blockCopy(cursorCol, cursorRow, rightMargin - destCol, 1, destCol, cursorRow);
        }

        int offsetDueToCombiningChar = ((displayWidth <= 0 && cursorCol > 0 && !aboutToAutoWrap) ? 1 : 0);
        screen.setChar(cursorCol - offsetDueToCombiningChar, cursorRow, codePoint, getStyle());

        if (autoWrap && displayWidth > 0)
            aboutToAutoWrap = (cursorCol == rightMargin - displayWidth);

        cursorCol = Math.min(cursorCol + displayWidth, rightMargin - 1);
    }
    ...
    public void reset() {
        ...
        rightMargin = columns;
        aboutToAutoWrap = false;
        foreColor = savedStateMain.savedForeColor = savedStateAlt.savedForeColor = TextStyle.COLOR_INDEX_FOREGROUND;
        backColor = savedStateMain.savedBackColor = savedStateAlt.savedBackColor = TextStyle.COLOR_INDEX_BACKGROUND;
        setDefaultTabStops();

        useLineDrawingG0 = useLineDrawingG1 = false;
        useLineDrawingUsesG0 = true;

        savedStateMain.savedCursorRow = savedStateMain.savedCursorCol = savedStateMain.savedEffect = savedStateMain.savedDecFlags = 0;
        savedStateAlt.savedCursorRow = savedStateAlt.savedCursorCol = savedStateAlt.savedEffect = savedStateAlt.savedDecFlags = 0;
        ...
        savedDecSetFlags = savedStateMain.savedDecFlags = savedStateAlt.savedDecFlags = currentDecSetFlags;
    ...
    public void paste(String text) {
        ...
        if (bracketed) session.write("\033[200~");
        session.write(text);
        if (bracketed) session.write("\033[201~");
    }
    ...
    static final class SavedScreenState {
        ...
        int savedEffect, savedForeColor, savedBackColor;
        int savedDecFlags;
        boolean useLineDrawingG0, useLineDrawingG1, useLineDrawingUsesG0 = true;
    }

    @Override
    public String toString() {
        return "TerminalEmulator[size=" + screen.columns + "x" + screen.screenRows + ", margins={" + topMargin + "," + rightMargin + "," + bottomMargin
            + "," + leftMargin + "}]";
    }
